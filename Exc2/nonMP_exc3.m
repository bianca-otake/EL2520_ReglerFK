...
figure(1); clf; sigma(G_nonmp);
% hinfvalue = hinfnorm(mp_min); % different scales but the values are the same
grid on; title('Singular Values non Minimum Phase')
...
[num,den] = tfdata(G_nonmp);  % useful, maybe?
...
% % minphase
RGA = G_mp .* inv(G_mp)';
figure(); bodemag(RGA); grid on; title('Bodemag of RGA minimium phase')
...
evalfr(RGA_nonmin,0)
figure(); bodemag(RGA_nonmin); grid on; title('Bodemag of RGA non minimium phase')
...
[m p] = bode(f1*G_nonmp(1,2),wc);
K1 = K1/m; f1 = K1*(1+1/(s*T1));
% figure(31); bode(f1*G_nonmp(1,2));  grid on

K2 = 1; 
T2 = 1;
f2 = K2*(1+1/(s*T2));

[m p] = bode(G_nonmp(2,1),wc);
...
[m p] = bode(f2*G_nonmp(2,1),wc);
K2 = K2/m; f2 = K2*(1+1/(s*T2));
% figure(32); bode(f2*G_nonmp(2,1));  grid on;

F = [0 f1;f2 0];

figure(321); clf; 
subplot(1,2,1);  bode(f1*G_nonmp(1,2)); title('Bode f1*G(1,2)'); grid on
subplot(1,2,2); bode(f2*G_nonmp(2,1)); title('Bode f2*G(2,1)')
grid on;
lines = findobj(gcf,'Type','Line');
for i = 1:numel(lines)
  lines(i).LineWidth = 2.0;
